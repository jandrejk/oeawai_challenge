...
class BasicBlock3x3_fft(nn.Module):
    expansion = 1

    def __init__(self, inplanes3_fft, planes, stride=1, downsample=None):
        super(BasicBlock3x3_fft, self).__init__()
        self.conv1 = conv3x3(inplanes3_fft, planes, stride)
        self.bn1 = nn.BatchNorm1d(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes)
        self.bn2 = nn.BatchNorm1d(planes)
...
class BasicBlock5x5_fft(nn.Module):
    expansion = 1

    def __init__(self, inplanes5_fft, planes, stride=1, downsample=None):
        super(BasicBlock5x5_fft, self).__init__()
        self.conv1 = conv5x5(inplanes5_fft, planes, stride)
        self.bn1 = nn.BatchNorm1d(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv5x5(planes, planes)
        self.bn2 = nn.BatchNorm1d(planes)
...
class BasicBlock7x7_fft(nn.Module):
    expansion = 1

    def __init__(self, inplanes7_fft, planes, stride=1, downsample=None):
        super(BasicBlock7x7_fft, self).__init__()
        self.conv1 = conv7x7(inplanes7_fft, planes, stride)
        self.bn1 = nn.BatchNorm1d(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv7x7(planes, planes)
        self.bn2 = nn.BatchNorm1d(planes)
...
class FFTMSResNet(nn.Module):
    def __init__(self, input_channel, device, layers=[1, 1, 1, 1], num_classes=10):
        self.device = device
        self.inplanes3 = 64
        self.inplanes5 = 64
        self.inplanes7 = 64
        self.inplanes3_fft = 64
        self.inplanes5_fft = 64
        self.inplanes7_fft = 64

        super(FFTMSResNet, self).__init__()
        ...
        self.conv1_fft = nn.Conv1d(input_channel, 64, kernel_size=7, stride=2, padding=3,
                               bias=False)
        self.bn1_fft = nn.BatchNorm1d(64)
        self.relu_fft = nn.ReLU(inplace=True)
        self.maxpool_fft = nn.MaxPool1d(kernel_size=3, stride=2, padding=1)
        ...
        self.layer3x3_1_fft = self._make_layer3_fft(BasicBlock3x3_fft, 64, layers[0], stride=2) # fft
        self.layer3x3_2_fft = self._make_layer3_fft(BasicBlock3x3_fft, 128, layers[1], stride=2)
        self.layer3x3_3_fft = self._make_layer3_fft(BasicBlock3x3_fft, 256, layers[2], stride=2)
        self.layer3x3_4_fft = self._make_layer3_fft(BasicBlock3x3_fft, 512, layers[3], stride=2)
        ...
        self.layer5x5_1_fft = self._make_layer5_fft(BasicBlock5x5_fft, 64, layers[0], stride=2)
        self.layer5x5_2_fft = self._make_layer5_fft(BasicBlock5x5_fft, 128, layers[1], stride=2)
        self.layer5x5_3_fft = self._make_layer5_fft(BasicBlock5x5_fft, 256, layers[2], stride=2)
        self.layer5x5_4_fft = self._make_layer5_fft(BasicBlock5x5_fft, 512, layers[3], stride=2)
        ...
        self.layer7x7_1_fft = self._make_layer7_fft(BasicBlock7x7_fft, 64, layers[0], stride=2)
        self.layer7x7_2_fft = self._make_layer7_fft(BasicBlock7x7_fft, 128, layers[1], stride=2)
        self.layer7x7_3_fft = self._make_layer7_fft(BasicBlock7x7_fft, 256, layers[2], stride=2)
        self.layer7x7_4_fft = self._make_layer7_fft(BasicBlock7x7_fft, 512, layers[3], stride=2)
    ...
    def _make_layer3_fft(self, block, planes, blocks, stride=2):
        downsample = None
        if stride != 1 or self.inplanes3_fft != planes * block.expansion:
            downsample = nn.Sequential(
                nn.Conv1d(self.inplanes3_fft, planes * block.expansion,
                          kernel_size=1, stride=stride, bias=False),
                nn.BatchNorm1d(planes * block.expansion),
            )

        layers = []
        layers.append(block(self.inplanes3_fft, planes, stride, downsample))
        self.inplanes3_fft = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes3_fft, planes))

        return nn.Sequential(*layers)

    def _make_layer5_fft(self, block, planes, blocks, stride=2):
        downsample = None
        if stride != 1 or self.inplanes5_fft != planes * block.expansion:
            downsample = nn.Sequential(
                nn.Conv1d(self.inplanes5_fft, planes * block.expansion,
                          kernel_size=1, stride=stride, bias=False),
                nn.BatchNorm1d(planes * block.expansion),
            )

        layers = []
        layers.append(block(self.inplanes5_fft, planes, stride, downsample))
        self.inplanes5_fft = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes5_fft, planes))

        return nn.Sequential(*layers)


    def _make_layer7_fft(self, block, planes, blocks, stride=2):
        downsample = None
        if stride != 1 or self.inplanes7_fft != planes * block.expansion:
            downsample = nn.Sequential(
                nn.Conv1d(self.inplanes7_fft, planes * block.expansion,
                          kernel_size=1, stride=stride, bias=False),
                nn.BatchNorm1d(planes * block.expansion),
            )

        layers = []
        layers.append(block(self.inplanes7_fft, planes, stride, downsample))
        self.inplanes7_fft = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes7_fft, planes))
    ...
    def forward(self, x0):
        ...
        x0_fft = torch.zeros(x0.size())
        for i,sample in enumerate(x0):
            x0_fft[i] = torch.from_numpy(ffts(sample)).to(self.device)  # fft
        ...
        x0_fft = self.bn1_fft(x0_fft)
        x0_fft = self.relu_fft(x0_fft)
        x0_fft = self.maxpool_fft(x0_fft)
